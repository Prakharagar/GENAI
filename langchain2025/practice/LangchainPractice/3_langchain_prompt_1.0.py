...
A PromptTemplate is a structured and reusable way to define prompts for language models (LLMs).

Instead of hardcoding long prompts or manually formatting them, PromptTemplate lets you:

Insert dynamic variables

Keep your prompts reusable and maintainable
...
prompt = f'''
You are an expert sales news analyst.

Analyze the sentiment of the following text. Respond with:
1. Give 3 Brief explanation for your sentiment judgment with example. 
2. The sentiment for each explainations: Positive, Neutral, or Negative.
3. A probability score (in %) for each sentiment type.
4. overall sentiments
5. overall probablity score

Text:
{document_text}

Format your answer like this:
Sentiment for each explanation: <Positive/Neutral/Negative>
Reason: <Why this sentiment was detected for each explanation>
Probability:
- Positive: <xx>%
- Neutral: <xx>%
- Negative: <xx>%
Overall sentiments
Overall probablity score
